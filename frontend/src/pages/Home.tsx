...
export const Home: React.FC = () => {
  ...
  const downloadQueryResult = (format: ExportFormat, result: QueryResult) => {
    if (!selectedDatabase) return;
  ...
  const confirmLargeExport = (result: QueryResult, onConfirm: () => void) => {
  ...
  const exportQueryResult = (format: ExportFormat, result = queryResult) => {
    if (!result || result.rows.length === 0) {
      message.warning("No data to export");
      return;
    }

    confirmLargeExport(result, () => downloadQueryResult(format, result));
  };

  const promptForExport = (result: QueryResult) => {
    ...
    modal = Modal.info({
      ...
      content: "Need to export this query result as CSV or JSON?",
      okText: "Not now",
      footer: (
        <Space>
          <Button onClick={() => modal.destroy()}>Not now</Button>
          <Button
            onClick={() => {
              exportQueryResult("json", result);
              modal.destroy();
            }}
          >
            Export JSON
  ...
  const handleExecuteQuery = async (exportFormat?: ExportFormat) => {
    ...
    try {
      ...
      if (exportFormat) {
        exportQueryResult(exportFormat, response.data);
      } else {
        promptForExport(response.data);
      }
    } catch (error: any) {
            ...
            activeTab === "manual" ? (
              <Space size={8}>
                ...
                <Dropdown
                  menu={{
                    items: [
                      { key: "csv", label: "Execute and export CSV" },
                      { key: "json", label: "Execute and export JSON" },
                    ],
                    onClick: ({ key }) => handleExecuteQuery(key as ExportFormat),
                  }}
                  disabled={executing}
                >
                  <Button
